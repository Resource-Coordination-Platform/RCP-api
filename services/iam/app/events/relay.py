...
publisher confirms. Runs as a daemon thread inside the service process.

Guarantees: an event row is only marked published after the broker
confirms it; rows are locked with FOR UPDATE SKIP LOCKED so multiple
service replicas can relay concurrently without duplication (the
event_id keeps consumers idempotent regardless).
"""
...
log = logging.getLogger("iam.outbox-relay")
